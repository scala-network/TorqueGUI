...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/torque-wallet-gui.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    MONEROD_EXEC=torqued.exe
fi
...
$QMAKE ../torque-wallet-gui.pro "$CONFIG" || exit
$MAKE || exit

# Copy torqued to bin folder
