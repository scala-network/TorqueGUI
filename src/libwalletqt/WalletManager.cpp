                                    ...
                                    const QString &language, NetworkType::Type nettype, quint64 kdfRounds)
{
    ...
    Stellite::Wallet * w = m_pimpl->createWallet(path.toStdString(), password.toStdString(),
                                                  language.toStdString(), static_cast<Stellite::NetworkType>(nettype), kdfRounds);
...
Wallet *WalletManager::openWallet(const QString &path, const QString &password, NetworkType::Type nettype, quint64 kdfRounds)
{
    ...
    Stellite::Wallet * w =  m_pimpl->openWallet(path.toStdString(), password.toStdString(), static_cast<Stellite::NetworkType>(nettype), kdfRounds);
...
Wallet *WalletManager::recoveryWallet(const QString &path, const QString &memo, NetworkType::Type nettype, quint64 restoreHeight, quint64 kdfRounds)
{
    ...
    Stellite::Wallet * w = m_pimpl->recoveryWallet(path.toStdString(), "", memo.toStdString(), static_cast<Stellite::NetworkType>(nettype), restoreHeight, kdfRounds);
    m_currentWallet = new Wallet(w);
    return m_currentWallet;
}
                                            ...
                                            quint64 restoreHeight, quint64 kdfRounds)
{
    ...
    Stellite::Wallet * w = m_pimpl->createWalletFromKeys(path.toStdString(), "", language.toStdString(), static_cast<Stellite::NetworkType>(nettype), restoreHeight,
                                              ...
                                              const QString &deviceName, quint64 restoreHeight, const QString &subaddressLookahead)
{
    ...
    Stellite::Wallet * w = m_pimpl->createWalletFromDevice(path.toStdString(), password.toStdString(), static_cast<Stellite::NetworkType>(nettype),
...
{
    return Stellite::Wallet::maximumAllowedAmount();
}
...
{
    return QString::fromStdString(Stellite::Wallet::displayAmount(amount));
}

quint64 WalletManager::amountFromString(const QString &amount) const
{
    return Stellite::Wallet::amountFromString(amount.toStdString());
}

quint64 WalletManager::amountFromDouble(double amount) const
{
    return Stellite::Wallet::amountFromDouble(amount);
}

bool WalletManager::paymentIdValid(const QString &payment_id) const
{
    return Stellite::Wallet::paymentIdValid(payment_id.toStdString());
}

bool WalletManager::addressValid(const QString &address, NetworkType::Type nettype) const
{
    return Stellite::Wallet::addressValid(address.toStdString(), static_cast<Stellite::NetworkType>(nettype));
}

bool WalletManager::keyValid(const QString &key, const QString &address, bool isViewKey,  NetworkType::Type nettype) const
{
    std::string error;
    if(!Stellite::Wallet::keyValid(key.toStdString(), address.toStdString(), isViewKey, static_cast<Stellite::NetworkType>(nettype), error)){
...
{
    return QString::fromStdString(Stellite::Wallet::paymentIdFromAddress(address.toStdString(), static_cast<Stellite::NetworkType>(nettype)));
}
...
{
    return Stellite::Utils::isAddressLocal(daemon_address.toStdString());
}
...
{
    Stellite::WalletManagerFactory::setLogLevel(logLevel);
}

void WalletManager::setLogCategories(const QString &categories)
{
    Stellite::WalletManagerFactory::setLogCategories(categories.toStdString());
}
...
QString WalletManager::checkUpdates(const QString &software, const QString &subdir) const
{
  ...
  const std::tuple<bool, std::string, std::string, std::string, std::string> result = Stellite::WalletManager::checkUpdates(software.toStdString(), subdir.toStdString());
  if (!std::get<0>(result))
    return QString("");
...
{
    m_pimpl =  Stellite::WalletManagerFactory::getWalletManager();
}
