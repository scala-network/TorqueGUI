                                    ...
                                    const QString &language, NetworkType::Type nettype, quint64 kdfRounds)
{
    ...
    Torque::Wallet * w = m_pimpl->createWallet(path.toStdString(), password.toStdString(),
                                                  language.toStdString(), static_cast<Torque::NetworkType>(nettype), kdfRounds);
...
Wallet *WalletManager::openWallet(const QString &path, const QString &password, NetworkType::Type nettype, quint64 kdfRounds)
{
    ...
    Torque::Wallet * w =  m_pimpl->openWallet(path.toStdString(), password.toStdString(), static_cast<Torque::NetworkType>(nettype), kdfRounds);
...
Wallet *WalletManager::recoveryWallet(const QString &path, const QString &memo, NetworkType::Type nettype, quint64 restoreHeight, quint64 kdfRounds)
{
    ...
    Torque::Wallet * w = m_pimpl->recoveryWallet(path.toStdString(), "", memo.toStdString(), static_cast<Torque::NetworkType>(nettype), restoreHeight, kdfRounds);
    m_currentWallet = new Wallet(w);
    return m_currentWallet;
}
                                            ...
                                            quint64 restoreHeight, quint64 kdfRounds)
{
    ...
    Torque::Wallet * w = m_pimpl->createWalletFromKeys(path.toStdString(), "", language.toStdString(), static_cast<Torque::NetworkType>(nettype), restoreHeight,
                                              ...
                                              const QString &deviceName, quint64 restoreHeight, const QString &subaddressLookahead)
{
    ...
    Torque::Wallet * w = m_pimpl->createWalletFromDevice(path.toStdString(), password.toStdString(), static_cast<Torque::NetworkType>(nettype),
...
{
    return Torque::Wallet::maximumAllowedAmount();
}
...
{
    return QString::fromStdString(Torque::Wallet::displayAmount(amount));
}

quint64 WalletManager::amountFromString(const QString &amount) const
{
    return Torque::Wallet::amountFromString(amount.toStdString());
}

quint64 WalletManager::amountFromDouble(double amount) const
{
    return Torque::Wallet::amountFromDouble(amount);
}

bool WalletManager::paymentIdValid(const QString &payment_id) const
{
    return Torque::Wallet::paymentIdValid(payment_id.toStdString());
}

bool WalletManager::addressValid(const QString &address, NetworkType::Type nettype) const
{
    return Torque::Wallet::addressValid(address.toStdString(), static_cast<Torque::NetworkType>(nettype));
}

bool WalletManager::keyValid(const QString &key, const QString &address, bool isViewKey,  NetworkType::Type nettype) const
{
    std::string error;
    if(!Torque::Wallet::keyValid(key.toStdString(), address.toStdString(), isViewKey, static_cast<Torque::NetworkType>(nettype), error)){
...
{
    return QString::fromStdString(Torque::Wallet::paymentIdFromAddress(address.toStdString(), static_cast<Torque::NetworkType>(nettype)));
}
...
{
    return Torque::Utils::isAddressLocal(daemon_address.toStdString());
}
...
{
    Torque::WalletManagerFactory::setLogLevel(logLevel);
}

void WalletManager::setLogCategories(const QString &categories)
{
    Torque::WalletManagerFactory::setLogCategories(categories.toStdString());
}
...
QString WalletManager::checkUpdates(const QString &software, const QString &subdir) const
{
  ...
  const std::tuple<bool, std::string, std::string, std::string, std::string> result = Torque::WalletManager::checkUpdates(software.toStdString(), subdir.toStdString());
  if (!std::get<0>(result))
    return QString("");
...
{
    m_pimpl =  Torque::WalletManagerFactory::getWalletManager();
}
